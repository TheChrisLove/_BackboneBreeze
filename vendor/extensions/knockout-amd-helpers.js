...
define(["knockout"], function(ko) {
    ...
    (function(ko, require) {
        ...
        engine.makeTemplateSource = function(template, doc, options) {
            var el;
            ...
            if (typeof template === "string") {
                ...
                if (!(template in sources)) {
                    var source;
                    if (options && options.templateKey) {
                        source = sources[options.templateKey] = new ko.templateSources.requireTemplate(options.templateKey);
                        source.retrieved = true;
                        source.requested = true;
                        source.template(template);
                    } else source = sources[template] = new ko.templateSources.requireTemplate(template);
                }
        ...
        engine.renderTemplate = function(template, bindingContext, options, templateDocument) {
            ...
            if (localTemplate && (typeof localTemplate === "function" || typeof localTemplate === "string")) {
                ...
                templateSource = engine.makeTemplateSource(template, templateDocument, options);
            }
